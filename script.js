...
случае, показывать шутку и картинку. */
     ...
     const facts = [
       "Когда программист видит баг, он не плачет — он устраивает с
ним «дуэль» и ищет, кто быстрее его победит. Побеждает тот, кто
быстрее перезагрузит компьютер.",
       "Настоящий программист может объяснить, почему его код — это
произведение искусства, а все остальные — «какой-то мусор». И при этом
сам не может объяснить, почему он написал именно так.",
       "«Hello, World!» — первый шаг программиста: практически каждый
новичок в программировании начинает с этой фразы. А некоторые до сих
пор не могут избавиться от привычки писать «Hello, World!» при каждом
новом проекте.",
      "Программистов можно узнать по тому, что их любимая фраза — «Это
не баг, это фича!» А иногда и вовсе — «Это не баг, это мой
эксперимент, который вышел из-под контроля».",
      "Если у вас есть друг-программист, он может часами обсуждать
разницу между «==» и «===», а вы вообще не поймёте, что он говорит. Но
зато он точно знает, как исправить ошибку за 3 секунды, которая у вас
занимает 3 дня.",
      "В жизни программиста есть только два состояния: «Загружено» и
«Перезагружено». А иногда — «Зависло и не реагирует».",
      "Когда программист говорит «Почему всё работает?», — это значит,
что он только что перезагрузил компьютер и надеется, что всё
исправится. Реально, магия.",
      "Настоящий программист может понять смысл любой ошибки за 2
секунды, а вот объяснить его — за две недели.",
      "Есть теория, что у программиста есть специальный язык: он
говорит на английском, а думает на двоичных кодах и переменных. И
иногда даже с кем-то общается только через комментарии в коде.",
      "Программистам свойственно устраивать «битвы» с компьютером: кто
кого — они или баги. Обычно выигрывают баги, потому что они
«затягиваются» больше, чем сериал.",
      "Когда программист говорит, что он «просто допиливает» код, —
это значит, что он уже сидит там три ночи и не знает, зачем вообще
сюда полез.",
       "Самая страшная вещь для программиста — это потерять свой
комментарий в коде. Потому что без него всё выглядит как загадка для
...
отдельной вкладке. */
 ...
 if (randomQuoteBtn) {
   randomQuoteBtn.addEventListener('click', () => {
     const quotes = [
       "«Программирование сегодня — это гонка разработчиков
программного обеспечения, стремящихся создать все более совершенные и
идиотоустойчивые программы, и вселенной, пытающейся создать все более
совершенных идиотов. Пока вселенная побеждает.» - Рич Кук",
       "«Сначала реши проблему. Потом пиши код.» – Джон Джонсон",
       "«Прежде чем отладить код, надо убедиться, что он вообще работает.»",
       "«Как понять, что ты хороший программист? Когда смотришь на
чужой код и думаешь: “Что за фигню я тут нагородил?»",
       "«Есть всего два сложных вопроса в информатике: инвалидация
кеша и присвоение имен.» - Фил Карлтон",
       "«Если бы строители строили так, как программисты пишут код, то
первая же прилетевшая птица разрушила бы цивилизацию.»",
       "«Оптимизм программиста: “Работает? Не трогай!” Пессимизм
программиста: “Работает? Нужно записать это где-то.»"
     ];
...
' или " для шаблонных строк. */
 ...
 if (programmingTipBtn) {
   programmingTipBtn.addEventListener('click', () => {
     const tips = [
       "Пишите комментарии к коду. Да, прямо сейчас! Даже если
кажется, что вы все помните. Через месяц вы будете благодарны себе, а
ваши коллеги – тем более. (И старайтесь писать комментарии, которые
объясняют зачем вы сделали что-то, а не просто что вы сделали.)",
       "Отлаживайте код, как будто вы играете в игру “Найди баг”.
Каждый найденный баг – это очко! И помните, иногда лучше сделать
перерыв, чем пялиться на код до посинения.",
       "Используйте систему контроля версий (например, Git).",
       "Используйте переменные с понятными именами. x, y, z – это для
математики. Для программирования используйте имяПользователя, возраст,
ценаТовара. Ваша будущая версия вас скажет вам спасибо.",
       "Не тратьте время на оптимизацию, если у вас нет проблем с
производительностью. Сначала сделайте так, чтобы работало, потом
сделайте это хорошо, а только потом – быстро.",
       "Помните, что программирование - это не только про код. Это еще
и про общение, решение проблем и умение искать информацию. И иногда,
про кофеин."
     ];
 ...
 if (funFactBtn) {
   funFactBtn.addEventListener('click', () => {
     const facts = [
       ...
       "Язык программирования Python назван в честь комик-группы
«Монти Пайтон».",
       "Самая большая в мире коллекция компьютерных вирусов,
насчитывающая более 20 миллионов, принадлежит России.",
       "В компьютерных играх больше полигонов, чем звезд в галактике.
Это говорит о технологическом прогрессе в сфере графики.",
       "Компьютер Apple I стоил $666.66. Стив Возняк, один из
основателей Apple, выбрал такую цену, потому что ему нравилось
повторяющееся число.",
       "Компьютеры вычисляют всё быстрее, кроме того, когда вам нужно,
чтобы они сделали это быстро. Это как закон подлости для технологий.",
       "Wi-Fi называется “беспроводным”, потому что он использует
провода в огромном количестве мест, кроме вашего устройства.",
       "Количество строк кода, необходимых, чтобы запустить тостер,
превышает количество строк кода, необходимых, чтобы отправить человека
в космос в 1960-х годах. Тостеры, оказывается, сложнее ракет.",
       "У разработчиков программного обеспечения есть любимое занятие
– исправление ошибок, которые они сами создали.",
       "Каждый раз, когда вы нажимаете кнопку “Забыли пароль?”, где-то
в мире один программист плачет. (Или радуется, что его работа еще
нужна.)",
       "Если бы все компьютеры, потребляющие энергию для майнинга
криптовалюты, объединились, они бы создали искусственное солнце,
которое сожгло бы Землю, чтобы заработать больше криптовалюты.
Прогресс, да?",
       "Самая большая угроза для безопасности данных – это люди,
которые используют “password” в качестве пароля. Гении
кибербезопасности."
     ];
...
function startGame() {
    ...
    level = 1;
    score = 0;
    instructions.textContent = "Готовы к новым вызовам? Чем больше
очков — тем круче!";
    alert("Окей, супергерой! Ты готов к битве чисел? Давай-ка проверим
твой талант!");
    startLevel();
}
...
function startLevel() {
  ...
  // Обновляем интерфейс: показываем информацию о текущем уровне,
попытках и диапазоне
  ...
  guessBtn.addEventListener('click', () => {
      if (attempts >= maxAttempts || levelCompleted) return; // если
попытки закончились или уровень пройден

      let guessStr = prompt(`Введите число от ${min} до
${max}:\nОсталось попыток: ${maxAttempts - attempts}\n(Если боитесь...
просто скажите "Я сдаюсь" и нажмите "Отмена")`);
      if (guessStr === null) {
          alert("Ну что ж, герой, уходите с честью! Возвращайтесь,
когда захотите снова бороться с числовыми монстрами!");
          return;
      }

      let guess = parseInt(guessStr.trim(), 10);
      if (isNaN(guess) || guess < min || guess > max) {
          alert("Это явно не число или оно вне диапазона. Попробуйте
еще раз, без фантазий!");
          return;
      }
...
${attempts} попыток(-ки)!\nВы — настоящий маг чисел!`);
          ...
          alert("Больше! Не сдавайтесь, у вас еще есть шанс поймать
это число за хвост!");
      } else {
          alert("Меньше! Не волнуйтесь, число не убежит — оно тут, под
вашей командой!");
      }
...
— король чисел и шуток!`);
          ...
          setTimeout(() => {
              alert("Готовы к следующему уровню? Там еще больше чисел
