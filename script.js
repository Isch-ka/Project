...
function startLevel() {
    ...
    maxAttempts = 5 + level; // попытки увеличиваются с уровнем
    attempts = 0;
    ...
    function updateAttemptsDisplay() {
        ...
        if (attemptsDisplay) {
            attemptsDisplay.textContent = `Осталось попыток: ${maxAttempts - attempts}`;
        }
    }
    ...
    gameStatusDiv.innerHTML = `
        ...
        <p id="attemptsDisplay">Осталось попыток: ${maxAttempts}</p>
    ...
    guessBtn.addEventListener('click', () => {
        if (attempts >= maxAttempts || levelCompleted) return;

        let guessStr = prompt(`Введите число от ${min} до ${max}:\nОсталось попыток: ${maxAttempts - attempts}\n(Если боитесь... просто скажите "Я сдаюсь" и нажмите "Отмена")`);
        if (guessStr === null) {
        ...
        attempts++;
        // Обновляем отображение попыток на странице
        updateAttemptsDisplay();

        // Проверка правильности догадки
        if (guess === secretNumber) {
            alert(`🎉 Браво! Вы угадали число ${secretNumber} за ${attempts} попыток(ки)!\nВы — настоящий маг чисел!`);
        ...
        if (attempts >= maxAttempts && !levelCompleted) {
            alert(`Увы, попытки кончились! Загаданное число было ${secretNumber}. Но не расстраивайтесь, следующий уровень — ваше время сиять!`);
            nextLevel(false);
        }
    });
}

// Функция nextLevel() остается без изменений
function nextLevel(success = true) {
    ...
    if (success) {
        ...
        level++;
        if (level > maxLevel) {
            alert(`🎉 Вы прошли все уровни! Итоговые очки: ${score}. Вы — король чисел и шуток!`);
        ...
        if (confirm(`Хочешь перепройти уровень ${level}? Или уже боишься?`)) {
            ...
            alert(`Ну что ж, герой, спасибо за игру! Ты — настоящий чемпион чисел! До новых встреч!`);
        }
    }
}
